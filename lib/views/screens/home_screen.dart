...
import 'package:dars_29_home/views/widgets/search_course.dart';
      ...
      appBar: AppBar(
        actions: [
          FilledButton(onPressed: () async{
            List<Course> courses = await coursesController.getCourses();
            Map<String,dynamic> mp = {
              'courses': courses,
              'todos':todos,
              'alarms':alarms
            };
            await showSearch(context: context, delegate: SearchViewDelegate(mp));
          }, child: Icon(Icons.search)),
          FilledButton(onPressed: (){
            setState(() {
              isList = !isList;
            });
          }, child: isList ? Icon(Icons.list,color: AppBarTheme.of(context).iconTheme?.color,) : Icon(Icons.candlestick_chart))
        ],
      ),
      drawer: CustomDrawer(),
